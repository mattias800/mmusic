...
public static class DownloadQueueFactory
{
    ...
    )
    {
        ...
        logger.Info(
            $"[QueueFactory] Begin create for {artistName} - {releaseTitle} (expectedCount={(expectedTrackCount?.ToString() ?? "null")}) from user={searchResponse.Username}"
        );
        var nArtist = NormalizeForCompare(artistName);
        var nTitle = NormalizeForCompare(releaseTitle);

        var annotated = searchResponse
            .Files.Where(file =>
        ...
        try
        {
            logger.Info($"[QueueFactory] Annotated candidate files={annotated.Count}");
        }
        catch { }
        if (annotated.Count == 0)
        {
            try
            {
                logger.Warn("[QueueFactory] No MP3 320 files in response");
            }
            catch { }
        ...
        foreach (var a in annotated)
        {
            ...
            if (idxArtist < 0)
            {
                ...
                {
                    logger.Info(
                        $"[QueueFactory] Reject path (no artist segment match): {a.NormalizedPath}"
                    );
                }
                catch { }
                continue;
            ...
            if (idxAlbum < 0)
            {
                ...
                {
                    logger.Info(
                        $"[QueueFactory] Reject path (no title segment match after artist): {a.NormalizedPath}"
                    );
                }
                catch { }
                continue;
        ...
        try
        {
            ...
            {
                logger.Info(
                    $"[QueueFactory] Group albumKey='{kv.Key.albumKey}' disc={(kv.Key.disc?.ToString() ?? "null")} count={kv.Value.Count}"
                );
            }
        }
        catch { }
        ...
        var filtered =
            (expectedTitlesList.Count > 0)
                ? inPrimary.Where(a => titleIsExpected(a)).ToList()
                : inPrimary.Where(a => !PathContainsHints(a, exclusionHints)).ToList();
        try
        {
            logger.Info(
                $"[QueueFactory] Filtered inPrimary: before={inPrimary.Count} after={filtered.Count} mode={(expectedTitlesList.Count > 0 ? "expected-titles" : "hint-exclusion")}"
            );
            if (filtered.Count == 0 && inPrimary.Count > 0)
            {
                foreach (var a in inPrimary.Take(10))
                {
                    ...
                    bool exp =
                        expectedTitlesList.Count > 0
                        && expectedTitlesList.Any(t => AreTitlesEquivalent(baseNoQualifiers, t));
                    bool hinted = PathContainsHints(a, exclusionHints);
                    logger.Info(
                        $"[QueueFactory] Rejected: file='{a.JustFile}' expectedMatch={exp} hinted={hinted}"
                    );
                }
            }
        }
        ...
        int fallbackMin =
            expectedTrackCount.HasValue && expectedTrackCount.Value > 0
        ...
        if (filtered.Count == 0)
        {
            ...
            {
                logger.Warn(
                    "[QueueFactory] Filter produced zero items; falling back to inPrimary/annotated"
                );
            }
            catch { }
        ...
        try
        {
            ...
            {
                logger.Info(
                    $"[QueueFactory] Order: file='{x.a.JustFile}' rawLead={(x.rawLead?.ToString() ?? "null")} lead={(x.lead?.ToString() ?? "null")}"
                );
            }
        }
        catch { }
        ...
        if (
            ...
            && ordered.Count > expectedTrackCount.Value
        )
        {
        ...
        var queueItems = orderedAnnotated
            .Select(
                (x, i) =>
                {
                    ...
                    var baseName = string.IsNullOrWhiteSpace(x.a.SafeBaseName)
                        ? $"track_{trackNo}"
                        : x.a.SafeBaseName;
                    var local = $"{indexPrefix} - {baseName}{x.a.Extension}";
                    return new DownloadQueueItem(
                        searchResponse.Username,
                        x.a.OriginalFileName,
                        local,
                        x.lead
                    );
                }
    ...
    private static string RemoveLeadingTrackNumberPrefix(string name)
    {
        ...
        if (pos > start)
        {
            ...
            while (
                pos < span.Length
                && (span[pos] == '-' || span[pos] == '.' || span[pos] == '_' || span[pos] == ' ')
            )
            {
                pos++;
                // If it's a dash followed by a space, skip the space too
                if (pos < span.Length && span[pos - 1] == '-' && span[pos] == ' ')
                    pos++;
            }
    ...
    private static int? TryExtractDiscNumber(List<string> segments, int startIndex)
    {
        for (int i = startIndex; i < segments.Count; i++)
        {
            ...
            var m = Regex.Match(
                s,
                @"\b(?:cd|disc|disk|digital\s*media)[\s_-]*([0-9]+)\b",
    ...
    private static string StripQualifiers(string input)
    {
        ...
        result = Regex.Replace(
            result,
            @"\b(?:live|demo|instrumental|karaoke|remix|re[-\s]?recorded|bonus)\b",
            " ",
            RegexOptions.IgnoreCase
        );
    ...
    private static bool AreTitlesEquivalent(string a, string b)
    {
        ...
        if (na.Length >= 5 && nb.Contains(na, StringComparison.OrdinalIgnoreCase))
            return true;
        if (nb.Length >= 5 && na.Contains(nb, StringComparison.OrdinalIgnoreCase))
            return true;
        return false;
    }
}
