        ...
        try
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(path))
                {
                    ...
                    qblog.Info(
                        $"Startup: enabled={settings.EnableQBittorrentDownloader} baseUrlPresent={!string.IsNullOrWhiteSpace(qOpt.BaseUrl)} usernamePresent={!string.IsNullOrWhiteSpace(qOpt.Username)} savePathPresent={!string.IsNullOrWhiteSpace(qOpt.SavePath)}"
                    );
                }
                logger.LogInformation(
                    "[Startup] qBittorrent: enabled={Enabled} baseUrlPresent={Base} usernamePresent={User} savePathPresent={Save}",
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(path))
                {
                    ...
                    sablog.Info(
                        $"Startup: enabled={settings.EnableSabnzbdDownloader} baseUrlPresent={!string.IsNullOrWhiteSpace(sOpt.BaseUrl)} apiKeyPresent={!string.IsNullOrWhiteSpace(sOpt.ApiKey)} category={(sOpt.Category ?? string.Empty)}"
                    );
                }
                logger.LogInformation(
                    "[Startup] SABnzbd: enabled={Enabled} baseUrlPresent={Base} apiKeyPresent={Key}",
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(path))
                {
                    ...
                    plog.Info(
                        $"Startup: baseUrlPresent={!string.IsNullOrWhiteSpace(pOpt.BaseUrl)} apiKeyPresent={!string.IsNullOrWhiteSpace(pOpt.ApiKey)} timeoutSec={pOpt.TimeoutSeconds} retries={pOpt.MaxRetries}"
                    );
                }
                logger.LogInformation(
                    "[Startup] Prowlarr: baseUrlPresent={Base} apiKeyPresent={Key}",
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(path))
                {
                    ...
                    slog.Info(
                        $"Startup: downloaderEnabled={settings.EnableSoulSeekDownloader} librarySharingEnabled={settings.SoulSeekLibrarySharingEnabled} host={settings.SoulSeekHost} port={settings.SoulSeekPort} usernamePresent={!string.IsNullOrWhiteSpace(settings.SoulSeekUsername)} passwordPresent={!string.IsNullOrWhiteSpace(so.Password)}"
                    );
                }
                logger.LogInformation(
                    "[Startup] SoulSeek: downloaderEnabled={Enabled} librarySharingEnabled={Share} host={Host} port={Port} usernamePresent={User}",
