...
)
{
    public async Task<Result> Handle(Command command)
    {
        try
        {
            ...
            var primaryArtistGroups = allReleaseGroup
                .Where(rg =>
                {
            ...
            var importantCollaborations = allReleaseGroup
                .Where(rg =>
                {
                    var credits = rg.Credits?.ToList();
                    if (credits == null || credits.Count == 0)
                    ...
                    if (primaryCredit.Artist.Id == command.ArtistMbId)
                        return false;
            ...
            logger.LogInformation(
                "Filtered to {PrimaryArtistCount} primary artist release groups and {ImportantCollaborationCount} important collaborations",
                primaryArtistGroups.Count,
                importantCollaborations.Count
            );
            logger.LogInformation(
                "Final import count: {FinalCount} release groups",
