    ...
    )
    {
        ...
        while (true)
        {
            ...
            catch (Exception ex) when (attempt < maxAttempts)
            {
                log?.LogWarning(
                    ex,
                    "Retrying {Operation} (attempt {Attempt}/{MaxAttempts}) after {DelayMs}ms",
                    opName ?? "operation",
                    attempt,
                    maxAttempts,
                    delay
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        logger.LogDebug(
                            "[LibraryImportWorker] Library manifest missing; pausing processing."
                        );
                ...
                var progress =
                    scope.ServiceProvider.GetRequiredService<CurrentArtistImportStateService>();
                ...
                var cache =
                    scope.ServiceProvider.GetRequiredService<ServerLibrary.Cache.ServerLibraryCache>();
                var events =
                    scope.ServiceProvider.GetRequiredService<HotChocolate.Subscriptions.ITopicEventSender>();
                var db = scope.ServiceProvider.GetRequiredService<EventDbContext>();
                var importHistory =
                    scope.ServiceProvider.GetRequiredService<ImportHistoryService>();
                ...
                if (
                    item.JobKind == ArtistImportJobKind.RefreshReleaseMetadata
                ...
                )
                {
                    ...
                    logger.LogInformation(
                        "[LibraryImportWorker] Refreshing release metadata for {Artist}/{Folder}",
                        artistId,
                        releaseFolderName
                    );
                    ...
                    {
                        logger.LogWarning(
                            "[LibraryImportWorker] Release not found: {Artist}/{Folder}",
                            artistId,
                            releaseFolderName
                        );
                        continue;
                    ...
                    {
                        logger.LogWarning(
                            "[LibraryImportWorker] Artist missing MBID: {Artist}",
                            artistId
                        );
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        var artistFolderPath =
                            Path.GetDirectoryName(release.ReleasePath)
                            ?? Path.Combine(
                                (
                                    await scope
                                        .ServiceProvider.GetRequiredService<ServerSettingsAccessor>()
                                        .GetAsync()
                                ).LibraryPath,
                                artistId
                            );

                        string? releaseGroupId = release
                            .JsonRelease
                            .Connections
                        ...
                        {
                            var rgs = await WithRetryAsync(
                                () => mb.GetReleaseGroupsForArtistAsync(mbidForRefresh),
                                opName: "GetReleaseGroupsForArtist",
                                log: logger
                            );
                            ...
                            {
                                logger.LogWarning(
                                    "[LibraryImportWorker] No RG match for {Artist}/{Folder}",
                                    artistId,
                                    releaseFolderName
                                );
                                continue;
                        ...
                        else
                        {
                            ...
                            logger.LogInformation(
                                "[LibraryImportWorker] Fetching primary type for existing release group {ReleaseGroupId}",
                                releaseGroupId
                            );
                            try
                            {
                                var releaseGroup = await WithRetryAsync(
                                    () => mb.GetReleaseGroupByIdAsync(releaseGroupId),
                                    opName: "GetReleaseGroupById",
                                    log: logger
                                );
                                if (releaseGroup != null)
                                {
                                    ...
                                    logger.LogInformation(
                                        "[LibraryImportWorker] Found primary type: {PrimaryType} for release group {ReleaseGroupId}",
                                        primaryType,
                                        releaseGroupId
                                    );
                                }
                                ...
                                {
                                    logger.LogWarning(
                                        "[LibraryImportWorker] Could not fetch release group {ReleaseGroupId} from MusicBrainz",
                                        releaseGroupId
                                    );
                                }
                            }
                            catch (Exception ex)
                            {
                                logger.LogWarning(
                                    ex,
                                    "[LibraryImportWorker] Error fetching release group {ReleaseGroupId} from MusicBrainz",
                                    releaseGroupId
                                );
                            }
                        }
                        ...
                        var releaseImporter =
                            scope.ServiceProvider.GetRequiredService<LibraryReleaseImportService>();
                        if (!string.IsNullOrWhiteSpace(releaseGroupId))
                        {
                            await WithRetryAsync(
                                () =>
                        ...
                        await WithRetryAsync(
                            () => cache.UpdateReleaseFromJsonAsync(artistId, releaseFolderName),
                            opName: "UpdateReleaseFromJson",
                            log: logger
                        );
                        var updated = await WithRetryAsync(
                            () =>
                                cache.GetReleaseByArtistAndFolderAsync(artistId, releaseFolderName),
                            opName: "GetReleaseByArtistAndFolder",
                            log: logger
                        );
                        if (updated != null)
                        {
                    ...
                    catch (Exception ex)
                    {
                        logger.LogError(
                            ex,
                            "[LibraryImportWorker] Error refreshing {Artist}/{Folder}",
                            artistId,
                            releaseFolderName
                        );
                        try
                ...
                try
                {
                    if (
                        ...
                        && !string.IsNullOrWhiteSpace(item.ExternalArtistId)
                    )
                    {
                        // ExternalArtistId is typically a Spotify artist id; do not treat it as a MusicBrainz id.
                        logger.LogDebug(
                            "External artist id present for '{Artist}' (extId='{ExtId}'). Will resolve MBID via search instead of using external id.",
                            item.ArtistName,
                            item.ExternalArtistId
                        );
                    }

                    if (
                        string.IsNullOrWhiteSpace(mbArtistId)
                        && !string.IsNullOrWhiteSpace(item.SongTitle)
                    )
                    {
                        logger.LogDebug(
                            "Attempting to resolve MBID via recording search for song '{Song}'",
                            item.SongTitle
                        );
                        var recs = await mb.SearchRecordingByNameAsync(item.SongTitle!);
                        var artists = recs.SelectMany(r =>
                                r.Credits?.Select(c => c.Artist)?.Where(a => a != null) ?? []
                            )
                            .ToList();
                        var targetNorm = NormalizeArtistName(item.ArtistName);
                        var match = artists.FirstOrDefault(a =>
                            NormalizeArtistName(a!.Name) == targetNorm
                        );
                        mbArtistId = match?.Id;
                    }
                    ...
                    if (string.IsNullOrWhiteSpace(mbArtistId))
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(mbArtistId))
                        {
                            ...
                            {
                                logger.LogDebug(
                                    "Retrying MBID resolution with alt name '{AltName}'",
                                    alt
                                );
                                var altCandidates = await mb.SearchArtistByNameAsync(alt, 10, 0);
                                var altExact = altCandidates.FirstOrDefault(c =>
                                    NormalizeArtistName(c.Name) == targetNorm
                                );
                ...
                catch (Exception ex)
                {
                    logger.LogWarning(
                        ex,
                        "Failed to resolve MusicBrainz artist for {Artist}",
                        item.ArtistName
                    );
                }
                ...
                {
                    logger.LogWarning(
                        "Could not resolve MusicBrainz artist id for '{Artist}' - skipping",
                        item.ArtistName
                    );
                    progress.Set(
                        new ArtistImportProgress
                ...
                logger.LogInformation(
                    "Resolved MBID {MbArtistId} for artist '{Artist}'",
                    mbArtistId,
                    item.ArtistName
                );
                ...
                try
                {
                    ...
                    totalEligible = rgs.Count(rg =>
                        LibraryDecider.ShouldBeAddedWhenAddingArtistToServerLibrary(rg)
                    );
                    logger.LogDebug(
                        "Computed {Count} eligible release groups for artist '{Artist}'",
                        totalEligible,
                        item.ArtistName
                    );
                }
                catch (Exception ex)
                {
                    logger.LogWarning(
                        ex,
                        "Failed to pre-compute eligible release groups for '{Artist}'",
                        item.ArtistName
                    );
                }
                ...
                try
                {
                    ...
                    {
                        logger.LogError(
                            "Import failed for '{Artist}' (MBID {MbArtistId}): {Error}",
                            item.ArtistName,
                            mbArtistId,
                    ...
                    {
                        logger.LogInformation(
                            "Import completed for '{Artist}' (MBID {MbArtistId}). Updating cache and notifying subscribers.",
                            item.ArtistName,
                            mbArtistId
                        );
                        progress.Set(
                        ...
                        try
                        {
                            ...
                            {
                                logger.LogInformation(
                                    "Imported artist now present in cache: {ArtistName} (Id {ArtistId})",
                                    importedArtist.Name,
                                    importedArtist.Id
                                );
                                await events.SendAsync(
                                ...
                                await events.SendAsync(
                                    ServerLibrary.Subscription.LibrarySubscription.LibraryArtistUpdatedTopic(
                                        importedArtist.Id
                                    ),
                                    new Artist(importedArtist)
                                );
                                ...
                                {
                                    logger.LogDebug(
                                        "Attempting to link playlist items by external artist id {ExtId}",
                                        item.ExternalArtistId
                                    );
                                    var playlists = await db
                                        ...
                                        .Where(p =>
                                            p.Items.Any(i =>
                                                i.LocalArtistId == null
                                                && i.ExternalArtistId == item.ExternalArtistId
                                            )
                                        )
                                        .ToListAsync();
                                    ...
                                    foreach (var pl in playlists)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            logger.LogInformation(
                                                "Linked playlist item {PlaylistItemId} in playlist {PlaylistId} to artist {ArtistId}",
                                                it.Id,
                                                pl.Id,
                                                importedArtist.Id
                                            );
                                            await events.SendAsync(
                                                PlaylistSubscription.PlaylistItemUpdatedTopic(
                                                    pl.Id
                                                ),
                                                new PlaylistSubscription.PlaylistItemUpdatedMessage(
                                                    pl.Id,
                                                    it.Id
                                                )
                                            );
                                ...
                                if (
                                    ...
                                    && !string.IsNullOrWhiteSpace(item.PlaylistItemId)
                                )
                                {
                                    var playlist = await db
                                        .Playlists.Include(p => p.Items)
                                        .FirstOrDefaultAsync(p => p.Id == item.PlaylistId);
                                    var pi = playlist?.Items.FirstOrDefault(i =>
                                        i.Id == item.PlaylistItemId
                                    );
                                    if (
                                        pi is not null
                                        && string.IsNullOrWhiteSpace(pi.LocalArtistId)
                                    )
                                    {
                                        pi.LocalArtistId = importedArtist.Id;
                                        logger.LogInformation(
                                            "Backfilled specific playlist item {PlaylistItemId} in playlist {PlaylistId} to artist {ArtistId}",
                                            pi.Id,
                                            playlist!.Id,
                                            importedArtist.Id
                                        );
                            ...
                            {
                                logger.LogWarning(
                                    "Artist '{Artist}' was imported but not found in cache immediately after update.",
                                    item.ArtistName
                                );
                            }
                        }
                        catch (Exception ex)
                        {
                            logger.LogError(
                                ex,
                                "Error post-processing imported artist '{Artist}'",
                                item.ArtistName
                            );
                        }
