...
)
{
    ...
    public async Task<bool> AddNzbByUrlAsync(
        string nzbUrl,
        string? nzbName,
    ...
    )
    {
        ...
        try
        {
            ...
            logger.LogInformation(
                "[SABnzbd] addurl source NZB URL: {Url}",
                SanitizeUrlForLogs(nzbUrl)
            );
            serviceLogger.Info($"addurl source NZB URL: {SanitizeUrlForLogs(nzbUrl)}");

            var url =
                $"{baseUrl}/api?mode=addurl&name={Uri.EscapeDataString(nzbUrl)}&apikey={Uri.EscapeDataString(apiKey)}&output=json&cat={Uri.EscapeDataString(category)}";
            ...
            if (!resp.IsSuccessStatusCode)
            {
                ...
                logger.LogWarning(
                    "[SABnzbd] Add URL failed HTTP {Status}. Body: {Body}",
                    (int)resp.StatusCode,
                    trimmed
                );
            ...
            try
            {
                ...
                var okJson =
                    statusProp.ValueKind == System.Text.Json.JsonValueKind.True
                    || (
                ...
                {
                    logger.LogWarning(
                        "[SABnzbd] Add URL returned status=false. Body: {Body}",
                        body
                    );
    ...
    public async Task<bool> AddNzbByContentAsync(
        byte[] nzbBytes,
        string fileName,
        CancellationToken cancellationToken,
        string? pathOverride = null,
    ...
    )
    {
        ...
        {
            var url =
                $"{baseUrl}/api?mode=addfile&apikey={Uri.EscapeDataString(apiKey)}&output=json&cat={Uri.EscapeDataString(category)}";
            if (!string.IsNullOrWhiteSpace(nzbName))
            {
                url += "&nzbname=" + Uri.EscapeDataString(nzbName);
            }
            ...
            if (!ok)
            {
                ...
                if (!string.IsNullOrWhiteSpace(body) && body.Length > 500)
                    body = body[..500] + "…";
                logger.LogWarning(
                    "[SABnzbd] Add file failed HTTP {Status}. Body: {Body}",
                    (int)resp.StatusCode,
                    body
                );
    ...
    public async Task<bool> IsJobCompleteAsync(string nzbName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var queueUrl =
                $"{baseUrl}/api?mode=queue&apikey={Uri.EscapeDataString(apiKey)}&output=json";
            logger.LogInformation(
                "[SABnzbd] Checking queue for job '{JobName}' at {Url}",
                nzbName,
                queueUrl
            );
            ...
            if (queueResp.IsSuccessStatusCode)
            {
                ...
                logger.LogInformation(
                    "[SABnzbd] Queue API response (first 500 chars): {Body}",
                    preview
                );
                ...
                if (
                    queueJson.RootElement.TryGetProperty("queue", out var queue)
                ...
                )
                {
                    ...
                    foreach (var slot in slots.EnumerateArray())
                    {
                        if (slot.TryGetProperty("name", out var name))
                        {
                            ...
                            logger.LogInformation(
                                "[SABnzbd] Queue slot name: '{JobName}'",
                                jobName
                            );
                            serviceLogger.Info($"Queue slot name: '{jobName}'");

                            if (
                                jobName?.Contains(nzbName, StringComparison.OrdinalIgnoreCase)
                                == true
                            )
                            {
                                logger.LogInformation(
                                    "[SABnzbd] Found matching job in queue: '{JobName}'",
                                    jobName
                                );
                                ...
                                if (slot.TryGetProperty("status", out var status))
                                {
                                    ...
                                    var isComplete =
                                        statusStr == "completed" || statusStr == "finished";
                                    logger.LogInformation(
                                        "[SABnzbd] Job '{JobName}' status: '{Status}', complete: {Complete}",
                                        jobName,
                                        statusStr,
                                        isComplete
                                    );
                                    serviceLogger.Info(
                                        $"Job '{jobName}' status: '{statusStr}', complete: {isComplete}"
                                    );
                                    return isComplete;
                                }
                                else
                                {
                                    logger.LogWarning(
                                        "[SABnzbd] Job '{JobName}' found but no status property",
                                        jobName
                                    );
                                    serviceLogger.Warn(
                                        $"Job '{jobName}' found but no status property"
                                    );
                                }
                            }
                        }
            ...
            {
                logger.LogWarning(
                    "[SABnzbd] Queue check failed HTTP {Status}",
                    (int)queueResp.StatusCode
                );
            ...
            var historyUrl =
                $"{baseUrl}/api?mode=history&apikey={Uri.EscapeDataString(apiKey)}&output=json";
            logger.LogInformation(
                "[SABnzbd] Checking history for job '{JobName}' at {Url}",
                nzbName,
                historyUrl
            );
            ...
            if (historyResp.IsSuccessStatusCode)
            {
                ...
                var historyPreview =
                    historyBody.Length > 500 ? historyBody[..500] + "..." : historyBody;
                logger.LogInformation(
                    "[SABnzbd] History API response (first 500 chars): {Body}",
                    historyPreview
                );
                ...
                if (
                    historyJson.RootElement.TryGetProperty("history", out var history)
                    && history.TryGetProperty("slots", out var historySlots)
                )
                {
                    var historySlotCount = historySlots.GetArrayLength();
                    logger.LogInformation(
                        "[SABnzbd] Found {SlotCount} slots in history",
                        historySlotCount
                    );
                    ...
                    foreach (var slot in historySlots.EnumerateArray())
                    {
                        if (slot.TryGetProperty("name", out var name))
                        {
                            var jobName = name.GetString();
                            logger.LogInformation(
                                "[SABnzbd] History slot name: '{JobName}'",
                                jobName
                            );
                            ...
                            {
                                logger.LogInformation(
                                    "[SABnzbd] Job '{JobName}' found in history (completed)",
                                    jobName
                                );
            ...
            {
                logger.LogWarning(
                    "[SABnzbd] History check failed HTTP {Status}",
                    (int)historyResp.StatusCode
                );
                serviceLogger.Warn($"History check failed HTTP {(int)historyResp.StatusCode}");
            }

            logger.LogInformation(
                "[SABnzbd] Job '{JobName}' not found in queue or history",
                nzbName
            );
    ...
    )
    {
        ...
        try
        {
            ...
            // Look for our job in the queue
            if (
                json.RootElement.TryGetProperty("queue", out var queue)
                && queue.TryGetProperty("slots", out var slots)
            )
            {
                foreach (var slot in slots.EnumerateArray())
                {
                    if (
                        ...
                        && name.GetString()?.Contains(nzbName, StringComparison.OrdinalIgnoreCase)
                            == true
                    )
                    {
                        if (slot.TryGetProperty("status", out var status))
            ...
            if (
                json.RootElement.TryGetProperty("history", out var history)
                && history.TryGetProperty("slots", out var historySlots)
            )
            {
                foreach (var slot in historySlots.EnumerateArray())
