...
{
    Task<IReadOnlyList<ProwlarrRelease>> SearchAlbumAsync(
        string artistName,
        string releaseTitle,
        int? year,
        CancellationToken cancellationToken,
        IDownloadLogger? relLogger = null
    );
    Task<(bool ok, string message)> TestConnectivityAsyncPublic(
        CancellationToken cancellationToken
    );
...
) : IProwlarrClient
{
    ...
    private async Task<bool> TestConnectivityAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var timeoutCts = new CancellationTokenSource(
                TimeSpan.FromSeconds(connectivityTimeoutSeconds)
            );
        ...
        catch (TaskCanceledException ex) when (ex.InnerException is TaskCanceledException)
        {
            ...
            logger.LogWarning(
                "[Prowlarr] - Request was sent but no response received within {Timeout}s",
                connectivityTimeoutSeconds
            );
            logger.LogWarning(
                "[Prowlarr] - This suggests the server at {BaseUrl} is either:",
                options.Value.BaseUrl
            );
            logger.LogWarning("[Prowlarr]   * Not running (check if Prowlarr service is started)");
            logger.LogWarning(
                "[Prowlarr]   * Not accessible from this machine (check network/firewall)"
            );
            logger.LogWarning(
                "[Prowlarr]   * Overloaded and not responding (check server resources)"
            );
    ...
    private async Task<bool> TestSearchEndpointAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var testUrlBase =
                $"{baseUrl}/api/v1/search?apikey={Uri.EscapeDataString(apiKey)}&query=test&categories=3000&categories=3010&categories=3040&categories=3050";
            var indexers = options.Value.IndexerIds;
            var testUrl =
                testUrlBase
                + (
                    indexers is { Length: > 0 }
            ...
            {
                serviceLogger.Info(
                    $"Search test OK HTTP {(int)resp.StatusCode} in {dur.TotalSeconds:0.00}s"
                );
                return true;
            }
            else
    ...
    public async Task<(bool ok, string message)> TestConnectivityAsyncPublic(
        CancellationToken cancellationToken
    )
    {
        try
    ...
    public async Task<string> TestNetworkConnectivityAsync()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    info.Add(
                        $"TCP Connection: TIMEOUT - Could not connect to {host}:{port} within 5 seconds"
                    );
                }
            }
            catch (Exception ex)
            ...
            info.Add(
                "HTTP-level issues (like authentication, API errors) require additional testing."
            );
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            try
            {
                ...
                if (completedTask == connectTask)
                {
                    ...
                    info.Add(
                        $"   💡 This means the network path is working, but Prowlarr may not be responding"
                    );
                }
                else
                {
                    ...
                    info.Add(
                        $"   💡 SUGGESTION: Check if Prowlarr service is running and listening on port {port}"
                    );
                }
            }
            catch (Exception ex)
            {
                ...
                info.Add(
                    $"   💡 SUGGESTION: Check firewall rules and if Prowlarr is listening on port {port}"
                );
            }
    ...
    /// </summary>
    public async Task<string> DiagnoseConnectionIssueAsync(
    ...
    public async Task<string> GetStatusSummaryAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (IsConfigurationValid())
        {
            try
            {
                ...
                {
                    status.Add(
                        "Service is not healthy. Use DiagnoseConnectionIssueAsync() for details."
                    );
                }
            }
            catch (Exception ex)
    ...
    )
    {
        try
        {
            ...
            if (!isConnected)
            {
                return (
                    false,
                    "Prowlarr server is not accessible (network issue or authentication failed)"
                );
            }
    ...
    public async Task<IReadOnlyList<ProwlarrRelease>> SearchAlbumAsync(
        string artistName,
        string releaseTitle,
        int? year,
        CancellationToken cancellationToken,
    ...
    )
    {
        ...
        {
            logger.LogWarning(
                "[Prowlarr] Missing configuration: BaseUrl={BaseUrl}, ApiKey={HasApiKey}",
                baseUrl ?? "null",
        ...
        logger.LogInformation(
            "[Prowlarr] Original search: Artist='{Artist}', Release='{Release}', Year={Year}",
            artistName,
            releaseTitle,
            year?.ToString() ?? "null"
        );
        ...
        logger.LogInformation(
            "[Prowlarr] Broad base query (no year/quality): '{Query}'",
            baseBroadQuery
        );
        relLogger?.Info($"[Prowlarr] Broad base query (no year/quality): '{baseBroadQuery}'");

        var searchQueries = ProwlarrSearchStrategy
            .BuildQueries(artistName, releaseTitle, year, logger)
            .ToList();
        ...
        logger.LogInformation(
            "[Prowlarr] Will try search queries in order: {Queries}",
            string.Join(" | ", searchQueries)
        );
        relLogger?.Info(
            $"[Prowlarr] Will try search queries in order: {string.Join(" | ", searchQueries)}"
        );
        ...
        foreach (var searchQuery in searchQueries)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries + 1; attempt++)
            {
                ...
                {
                    logger.LogInformation(
                        "[Prowlarr] Executing search for query: '{Query}'",
                        searchQuery
                    );
                    relLogger?.Info($"[Prowlarr] Executing search for query: '{searchQuery}'");
                    var results = await TrySearchWithQueryAsync(
                        searchQuery,
                        artistName,
                        releaseTitle,
                        cancellationToken,
                    ...
                    logger.LogInformation(
                        "[Prowlarr] Query '{Query}' returned {Count} results",
                        searchQuery,
                        results.Count
                    );
                    relLogger?.Info(
                        $"[Prowlarr] Query '{searchQuery}' returned {results.Count} results"
                    );
                    ...
                    else
                    {
                        ...
                        {
                            logger.LogInformation(
                                "[Prowlarr] ❌ No results with broad search '{Query}'. Skipping narrower searches (year/320/FLAC).",
                                searchQuery
                            );
                            relLogger?.Info(
                                $"[Prowlarr] ❌ Broad search returned zero results; skipping narrower searches"
                            );
                            return Array.Empty<ProwlarrRelease>();
                        }

                        logger.LogInformation(
                            "[Prowlarr] ❌ No results with '{Query}', trying next quality level",
                            searchQuery
                        );
                        relLogger?.Info(
                            $"[Prowlarr] ❌ No results with '{searchQuery}', trying next quality level"
                        );
                ...
                catch (TaskCanceledException) when (attempt <= maxRetries)
                {
                    ...
                    logger.LogInformation(
                        "[Prowlarr] Waiting {Delay}s before retry...",
                        delaySeconds
                    );
                ...
                catch (Exception ex)
                {
                    logger.LogError(
                        ex,
                        "[Prowlarr] Final attempt failed for '{Query}' after {Attempt} attempts",
                        searchQuery,
                        attempt
                    );
        ...
        {
            relLogger?.Info(
                "[Prowlarr] Running post-failure diagnostics (connectivity + search endpoint)"
            );
    ...
    private async Task<IReadOnlyList<ProwlarrRelease>> TrySearchWithQueryAsync(
        string query,
        string artistName,
        string releaseTitle,
        CancellationToken cancellationToken,
    ...
    )
    {
        ...
        var candidateUrls = ProwlarrQueryBuilder.BuildCandidateUrls(
            baseUrl!,
            apiKey!,
            query,
            options.Value.IndexerIds,
            logger
        );
        ...
        foreach (var url in candidateUrls)
        {
            ...
            try
            {
                attemptNumber++;
                logger.LogInformation(
                    "[Prowlarr] ===== URL ATTEMPT #{Attempt}/{Total} =====",
                    attemptNumber,
                    candidateUrls.Count
                );
                relLogger?.Info(
                    $"[Prowlarr] ===== URL ATTEMPT #{attemptNumber}/{candidateUrls.Count} ====="
                );
                ...
                if (!result.Success)
                {
                    var delaySeconds = Math.Max(2, options.Value.RetryDelaySeconds);
                    logger.LogInformation(
                        "[Prowlarr] Waiting {Delay}s before next attempt...",
                        delaySeconds
                    );
                    try
                    {
                        relLogger?.Info(
                            $"[Prowlarr] Waiting {delaySeconds}s before next attempt..."
                        );
                    }
                    catch { }
                    await Task.Delay(TimeSpan.FromSeconds(delaySeconds), cancellationToken);
                ...
                logger.LogInformation(
                    "[Prowlarr] Response JSON length: {Length} characters",
                    json.Length
                );
                try
                {
                ...
                logger.LogInformation(
                    "[Prowlarr] Parsed {Count} results from response",
                    list.Count
                );
                ...
                {
                    logger.LogInformation(
                        "[Prowlarr] ✅ SUCCESS! Found {Count} results from this URL",
                        list.Count
                    );
                    relLogger?.Info(
                        $"[Prowlarr] ✅ SUCCESS! Found {list.Count} results from this URL"
                    );
                    return list;
                }
                else
                {
                    logger.LogInformation(
                        "[Prowlarr] ❌ No valid results from this URL variant - trying next"
                    );
                    relLogger?.Info(
                        "[Prowlarr] ❌ No valid results from this URL variant - trying next"
                    );
            ...
            catch (HttpRequestException ex)
            {
                ...
                if (ex.InnerException is SocketException sock)
                {
                    logger.LogWarning(
                        ...
                        url,
                        sock.SocketErrorCode,
                        (int)sock.SocketErrorCode,
                        ex.Message,
                        innerMsg ?? "<none>"
                    );
                }
                else
                {
                    logger.LogWarning(
                        ex,
                        "[Prowlarr] HTTP request failed for {Url}. Reason={Reason}. Inner={Inner}",
                        url,
                        ex.Message,
                        innerMsg ?? "<none>"
                    );
        ...
        logger.LogInformation(
            "Prowlarr returned no results for query '{Query}' across all URL variants. "
            ...
            options.Value.TestConnectivityFirst,
            GetConfigurationSummary(),
            await IsHealthyAsync(cancellationToken) ? "HEALTHY" : "UNHEALTHY",
            GetCurrentTimeout().TotalSeconds,
