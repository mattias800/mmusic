...
internal static class ProwlarrTextMatch
{
    public static bool TitleMatches(string? title, string artistName, string releaseTitle)
    {
        ...
        var artistMatchCount = artistWords.Count(word => titleWords.Any(x => FuzzyWordMatch(x, word)));
        if (artistMatchCount < Math.Max(1, artistWords.Length / 2)) return false;
        // Require at least half of album words
        var albumWords = album.Split(' ', StringSplitOptions.RemoveEmptyEntries);
        var albumMatchCount = albumWords.Count(word => titleWords.Any(x => FuzzyWordMatch(x, word)));
