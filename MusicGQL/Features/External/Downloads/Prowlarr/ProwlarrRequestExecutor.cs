...
internal sealed class ProwlarrRequestExecutor(
    HttpClient httpClient,
    IOptions<Configuration.ProwlarrOptions> options,
    ILogger logger
)
{
    ...
    public async Task<Result> GetJsonAsync(
        string url,
    ...
    )
    {
        ...
        if (options.Value.EnableDetailedLogging)
        {
            ...
            try
            {
                relLogger?.Info($"[Prowlarr] Headers: {headersStr}");
            }
            catch { }
        }
        ...
        logger.LogInformation(
            "[Prowlarr] RESPONSE: Status {Status} in {Duration:0.00}s",
            (int)resp.StatusCode,
        ...
        {
            relLogger?.Info(
                $"[Prowlarr] RESPONSE: Status {(int)resp.StatusCode} in {duration.TotalSeconds:0.00}s"
            );
        }
        catch { }
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            logger.LogInformation(
                "[Prowlarr] ❌ ERROR RESPONSE: HTTP {Status} {Reason} - {Body}",
                (int)resp.StatusCode,
                reasonPhrase,
                body ?? "null"
            );
