...
) : IHandleMessages<LookupReleaseInMusicBrainz>
{
    ...
    {
        logger.LogInformation("Looking up release with ID: {MusicBrainzReleaseId}", message.MusicBrainzReleaseId);

        var release = await service.GetReleaseByIdAsync(message.MusicBrainzReleaseId);
        if (release is null)
        {
            logger.LogWarning("Could not find release with ID: {MusicBrainzReleaseId}", message.MusicBrainzReleaseId);
