...
const mutation = graphql(`
  mutation UpdateProwlarrSettings($input: UpdateProwlarrSettingsInput!) {
    updateProwlarrSettings(input: $input) {
      ...
      ... on UpdateProwlarrSettingsError {
        message
      }
    }
  }
...
export const ProwlarrSettingsForm: React.FC = () => {
  ...
  const [timeout, setTimeout] = React.useState<number>(
    s?.prowlarrTimeoutSeconds ?? 30,
  );
  const [maxRetries, setMaxRetries] = React.useState<number>(
    s?.prowlarrMaxRetries ?? 2,
  );
  const [retryDelay, setRetryDelay] = React.useState<number>(
    s?.prowlarrRetryDelaySeconds ?? 1,
  );
  const [testFirst, setTestFirst] = React.useState<boolean>(
    s?.prowlarrTestConnectivityFirst ?? true,
  );
  const [detailed, setDetailed] = React.useState<boolean>(
    s?.prowlarrEnableDetailedLogging ?? false,
  );
  const [maxConc, setMaxConc] = React.useState<number>(
    s?.prowlarrMaxConcurrentRequests ?? 1,
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <div className="relative">
            <Input
              ...
              value={maxRetries}
              onChange={(e) =>
                setMaxRetries(parseInt(e.target.value || "0", 10))
              }
              className="pl-9 bg-white/5 border-white/10"
            />
        ...
        <div>
          ...
          <Input
            type="number"
            value={retryDelay}
            onChange={(e) => setRetryDelay(parseInt(e.target.value || "0", 10))}
            className="bg-white/5 border-white/10"
          />
        </div>
        <div>
      ...
      <div className="flex items-center gap-6">
        <label className="flex items-center gap-2 text-gray-300">
          <Switch
            checked={testFirst}
            onCheckedChange={setTestFirst}
            ariaLabel="Test connectivity first"
          />{" "}
        ...
        <label className="flex items-center gap-2 text-gray-300">
          <Switch
            ...
            ariaLabel="Enable detailed logging"
          />{" "}
      ...
      <p className="text-xs text-gray-400">
        API key should be provided via environment/secret store; not editable
        here.
      </p>
    </div>
  );
